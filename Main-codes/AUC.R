...
calc.AUC <- function(ids, anno,  cells_rankings, percent){
  res <- data.frame(lapply(ids, function(x){
    sub <- getset(x, anno)
    res <- AUC.cellSet(cellSet=sub, rankings=cells_rankings, aucMaxRank=ceiling(percent*nrow(cells_rankings))) 
    return(res)
  }))
  colnames(res) <- ids
...
calc.AUC3 <- function(ids, anno,  cells_rankings, N){
  res <- data.frame(lapply(ids, function(x){
    sub <- getset(x, anno)
    res <- AUC.cellSet2(cellSet=sub, rankings=cells_rankings, aucMaxRank=N) 
    return(res)
  }))
  colnames(res) <- ids
  res <- data.frame(t(res))
